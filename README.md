...
**P0 骨架已落地，P1 数据进行中**：`pnpm install` 后 `pnpm dev` 可打开窗口。Home → Ping；Data → Import / Export。虚表块走 Arrow IPC。50 万行 csv 可用 `python/.venv/Scripts/python.exe scripts/gen_large_csv.py` 生成（不要提交该文件）。

...
**P0 完成，P1 自动验收完成，P2 画布与 DataToManager 已落地**：`pnpm install` 后 `pnpm dev` 可打开窗口。Home → Ping；Data → Import / Export。虚表块走 Arrow IPC。左侧「节点」可拖 Constant / Output to DataManager，Ribbon「工作流 → 运行」执行后数据集列表会出现条目。dump/load wrap 与 undo 尚未做。

...
P2 任务 1–8 已落地。P3：已 vendor `dw_nodes_analysis/core`，工具箱含 Data Source（从 DataManager 取表）、Query 与 Drop NA。Ribbon Data → 清洗 → 删除缺失 与节点共用 `dropna_impl`。不要做图表、自由停靠或 Agent。表格单元格 undo 不与工作流栈合并。File 保存/打开仍属 P5。Ribbon Query 对话框与其余清洗节点仍属 P3 后续。

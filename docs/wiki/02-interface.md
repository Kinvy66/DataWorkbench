...
窗口是固定分区，不能像专业 CAD 那样把面板拖成任意停靠。记住下面五块就够用（编号与图中一致）。
...
| **主页** | 撤销、重做（只针对**工作流画布**的增删连线，不是改表格格子）；有的版本还有 Ping |
| **数据** | **添加数据**、移除、重命名、导出。清洗按钮不在这里 |
| **操作** | 清洗、过滤、统计（对**当前选中的那张表**立刻改，或生成新表） |
| **工作流** | 运行 / 停止积木图 |
| **图表** | 新建折线/散点/柱状/直方，标注，导出 PNG/SVG/PDF |

...
按顺序点，不要跳。示例表：仓库里的 `docs/wiki/samples/wiki-demo.csv`（11 行数据：`name,age,city,score`，含空格子和一行完全重复的 Alice）。

开始前窗口应已打开，底部日志有「Sidecar 已就绪」。还没开起来请先看 [01 安装与启动](./01-install.md)。

## 第 1 步：导入表（约 1 分钟）

1. 顶部 **数据 → 添加数据**。
2. 选 `docs/wiki/samples/wiki-demo.csv`。
...
1. **操作 → 数据描述** → 生成。
2. 左边多一张名字带 `describe` 的表。点它，中间变成统计数字（count、mean 等）。
3. 再点回原来的数据表（不要停在 describe 上），后面画图才画得到分数列。
...
更细的按钮说明：[03](./03-data.md)～[07](./07-project.md)。打钩验收：[09 手工测试清单](./09-test.md)。卡住：[10 常见问题](./10-faq.md)。

...
1. 双击安装程序。
2. 按向导「下一步」：同意许可。安装范围默认「仅当前用户」，目录默认在当前用户下（`%LOCALAPPDATA%\Programs\DataWorkbench`），**不必管理员**。`C:\Program Files` 是正规安装位置，没有功能问题，只是系统禁止普通用户往那里写文件。若要装到 Program Files：选「为所有用户安装」（会弹出 Windows 帐户控制），或在目录页改到 Program Files（同样会请求管理员权限；同意后向导可能从开头再走一遍）。
